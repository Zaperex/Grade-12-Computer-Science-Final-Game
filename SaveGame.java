  ...
  public void loadTroops(ArrayList<ArrayList<Terrain>> boardState, ArrayList<Troop> troops, String fileName) throws IOException{
    ...
    while ((line = troopReader.readLine()) != null){
      ...
      if (tempArr[0].equals("Footman")){
        troops.add(new Footman(tempArr[2], coordinates, "Footman.png")); //Creates object in correct coordinates
      ...
      //If troop is an archer
        ...
        troops.add(new Archer(tempArr[2], coordinates, "Archer.png")); //Creates object in correct coordinates
        troops.get(troops.size()-1).setHealth(Double.parseDouble(tempArr[1])); //Sets health to what they were at
        boardState.get(x).get(y).setTroop(troops.get(troops.size()-1)); //Stores troop into it's terrain tile
      }
      //If troop is a knight
      else if (tempArr[0].equals("Knight")){
        troops.add(new Knight(tempArr[2], coordinates, "Knight.png")); //Creates object in correct coordinates
        troops.get(troops.size()-1).setHealth(Double.parseDouble(tempArr[1])); //Sets health to what they were at
        boardState.get(x).get(y).setTroop(troops.get(troops.size()-1)); //Stores troop into it's terrain tile
      }
      //If troop is a crossbowmen
      else if (tempArr[0].equals("CrossbowMen")){
        troops.add(new CrossbowMen(tempArr[2], coordinates, "Crossbowman.png")); //Creates object in correct coordinates
        troops.get(troops.size()-1).setHealth(Double.parseDouble(tempArr[1])); //Sets health to what they were at
        boardState.get(x).get(y).setTroop(troops.get(troops.size()-1)); //Stores troop into it's terrain tile
      }
      //If troop is a calvary
      else if (tempArr[0].equals("Calvary")){
        troops.add(new Cavalry(tempArr[2], coordinates, "Cavalry.png")); //Creates object in correct coordinates
  ...
  public void loadBuildings(ArrayList<ArrayList<Terrain>> boardState, ArrayList<Building> buildings, String fileName) throws IOException{
    ...
    while ((line = buildReader.readLine()) != null){
      ...
      if (tempArr[0].equals("Castle")){
        ...
        int[] coordinates = {x, y}; //Stores the coordinates into an int array
        buildings.add(new Castle(tempArr[2], coordinates, "Castle.png")); //Creates object in correct coordinates
      ...
      //If building is Gold Mine
        ...
        int y = Integer.parseInt(tempArr[4]); //Y Coordinates
        int[] coordinates = {x, y}; //Stores the coordinates into an int array
        buildings.add(new GoldMine(tempArr[2], coordinates, "Gold Mine.png")); //Creates object in correct coordinates
        buildings.get(buildings.size()-1).setHealth(Double.parseDouble(tempArr[1])); //Sets health of building
      }
    }
  ...
  public boolean load(ArrayList<ArrayList<Terrain>> boardState, ArrayList<Building> unclaimedBuildings, String fileName) throws IOException{
    ...
    while((line = reader.readLine()) != null){
      ...
      //If player 2 turn found in file
        ...
        int y = Integer.parseInt(tempArr[4]); //Y Coordinates
        int[] coordinates = {x, y}; //Stores the coordinates into an int array
        unclaimedBuildings.add(new GoldMine(tempArr[2], coordinates, "Gold Mine.pn")); //Creates object in correct coordinates
