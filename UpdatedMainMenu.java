...
public class UpdatedMainMenu extends Application {
  Pane root = new Pane();
...
//    troopButtonLayout.getChildren().addAll(archerButton, knightButton, cbmButton, footManButton, calvaryButton);
    ...
    for (int i = 0; i < 7; i++) {
      for (int j = 0; j < 7; j++) {
        ...
        int[] coords = {j, i}; //Based on arraylist orientation with y controlling which row and x controlling which column
        int x = j;
        int y = i;
        
        if (j == 0 && i == 0) {
          terrain[j][i] = new Terrain("Swamp", coords);
          terrain[j][i].setBuilding(new Building("Home base", 100.0, 50.0, 100.0, "P1", coords, "Castle.png"));
        }
        else if (j == 6 && i == 6){
          
          //VALUES USED FOR TESTING *******************************
          terrain[j][i] = new Terrain("Plain", coords);
          terrain[j][i].setBuilding(new Building("Enemy Base", 100.0, 50.0, 100.0, "P2", coords, "Castle.png"));
        }
        else if (j == 3 && i == 3){
          terrain[j][i] = new Terrain("Plain", coords);
          terrain[j][i].setTroop(new Archer("P2", coords, "Archer.png"));
        }
        else if (j == 4 && i == 4){
          terrain[j][i] = new Terrain("Plain", coords);
          terrain[j][i].setTroop(new Knight("P1", coords, "Knight.png"));
        }
        else if (j == 3 && i == 5){
          terrain[j][i] = new Terrain("Plain", coords);
          terrain[j][i].setTroop(new Knight("P1", coords, "Knight.png"));
...
//                System.out.println(move[0] + " " + move[1]);
                button[move[1]][move[0]].setStyle("-fx-background-color: red;" + "-fx-text-fill: white");
              } 
            ...
            if ((x == 0 && y == 0 || x == 6 && y == 6) && terrain[x][y].getBuilding().getTeam().equals(turn)) {
...
//              button[j][i].addEventHandler(MouseEvent.MOUSE_CLICKED, new EventHandler<MouseEvent>() {
//                public void handle(MouseEvent e) {
//                  int[] oldCoords = terrain[y][x].getTroop().getCoords();
//                }
//              });
            }
          }
        ...
        if (terrain[j][i].getTroop() != null && terrain[j][i].getTroop().getImageView() != null) {
          //Adds the image of the troop onto the tile
          root.getChildren().add(terrain[j][i].getTroop().getImageView());
        }
        //Checks if there is a building 
        if (terrain[j][i].getBuilding() != null && terrain[j][i].getBuilding().getImageView() != null) {
          //Adds the image of the building onto the tile
          root.getChildren().add(terrain[j][i].getBuilding().getImageView());
        }
      }
    }
...
//    root.getChildren().add(troopButtonLayout); --------------------------------------------------------------------------
  ...
  public void combatScreen (Terrain[][] terrain) {
    
    Button attackButton = new Button("Attack");
    attackButton.setOnAction(e -> attackButtonClicked());
    Button blockButton = new Button("Block");
    blockButton.setOnAction(e -> blockButtonClicked());
    Button dodgeButton = new Button("Dodge");
    dodgeButton.setOnAction(e -> dodgeButtonClicked());
    Button specialButton = new Button("Special");
    specialButton.setOnAction(e -> specialButtonClicked());
    ...
    combatButtonLayout.relocate(125, 450);
    
    int attackerPositionX = 50;
    int attackerPositionY = 300;
    int defenderPositionX = 300;
    int defenderPositionY = 50;
    
    Rectangle player1 = new Rectangle(50, 100);
    player1.relocate(attackerPositionX, attackerPositionY);
    
    Rectangle player2 = new Rectangle(50, 100);
    player2.relocate(defenderPositionX, defenderPositionY);
    
    Pane root = new Pane();
    root.getChildren().add(combatButtonLayout);
    
    Scene combatScene = new Scene(root, 500, 500);
  ...
  public void dodgeButtonClicked () {
